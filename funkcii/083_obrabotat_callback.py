async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data.startswith("adm:report:"):
        ...
        text_limit = 50
        statuses = None
        if report_type == "stood":
            statuses = ["success"]
            text = build_report_stood(conn, limit=text_limit)
        elif report_type == "notstood":
            statuses = ["slip"]
            text = build_report_not_stood(conn, limit=text_limit)
        elif report_type == "general":
            statuses = ["success", "slip"]
            text = build_report_general(conn, limit=text_limit)
        else:
            text = "Отчёт не найден."
        if statuses:
            rows_all = _report_rows(conn, statuses, limit=None)
            if len(rows_all) > text_limit:
                csv_data = build_report_csv(rows_all)
